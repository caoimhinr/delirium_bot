...
`;
        ...
        // Step 2: Build prompt
        // ----------------------------
        const promptText = buildLLMPrompt(userRequest, documents);
        ...
        const generatedCode = azureRes.data.choices?.[0]?.message?.content?.trim() || "No code returned";

        // ----------------------------
        // Step 4: Send result
        // ----------------------------
        await replyMessage.edit(`\`\`\`csharp\n${generatedCode}\n\`\`\``);
